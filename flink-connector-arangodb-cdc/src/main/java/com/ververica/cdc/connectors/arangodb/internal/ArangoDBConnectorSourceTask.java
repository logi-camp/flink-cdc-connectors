...
import static com.ververica.cdc.connectors.arangodb.source.utils.CollectionDiscoveryUtils.bsonListToJson;
...
import static com.ververica.cdc.connectors.arangodb.source.utils.CollectionDiscoveryUtils.completionPattern;
...
import static com.ververica.cdc.connectors.arangodb.source.utils.CollectionDiscoveryUtils.includeListAsPatterns;
...
import static com.ververica.cdc.connectors.arangodb.source.utils.ArangoDBRecordUtils.isHeartbeatEvent;
import static com.ververica.cdc.connectors.arangodb.source.utils.ArangoDBRecordUtils.isSnapshotRecord;
 ...
 */
public class ArangoDBConnectorSourceTask extends SourceTask {
    ...
    public ArangoDBConnectorSourceTask() throws NoSuchFieldException {
    ...
    private SourceRecord markTimestampForDataRecord(SourceRecord record) {
        ...
        value.put(ArangoDBEnvelope.TIMESTAMP_KEY_FIELD, System.currentTimeMillis());
        ...
        long timestamp = 0L;
        if (value.schema().field(ArangoDBEnvelope.CLUSTER_TIME_FIELD) != null) {
            String clusterTime = value.getString(ArangoDBEnvelope.CLUSTER_TIME_FIELD);
            if (clusterTime != null) {
                timestamp = new JsonReader(clusterTime).readTimestamp().getTime() * 1000L;
            }
        }
        source.put(ArangoDBEnvelope.TIMESTAMP_KEY_FIELD, timestamp);
    ...
    private SourceRecord markTimestampForHeartbeatRecord(SourceRecord record) {
        ...
        heartbeatValue.put(ArangoDBEnvelope.TIMESTAMP_KEY_FIELD, Instant.now().toEpochMilli());
