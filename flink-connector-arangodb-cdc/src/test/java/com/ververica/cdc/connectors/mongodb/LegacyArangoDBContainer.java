...
public class LegacyArangoDBContainer extends GenericContainer<LegacyArangoDBContainer> {

    private static final Logger LOG = LoggerFactory.getLogger(LegacyArangoDBContainer.class);

    private static final String DOCKER_IMAGE_NAME = "mongo:5.0.2";

    public static final int ARANGODB_PORT = 8529;

    public static final String ARANGO_SUPER_USER = "root";

    public static final String ARANGO_SUPER_PASSWORD = "";
    ...
    public LegacyArangoDBContainer(Network network) {
        this(network, ShardingClusterRole.NONE);
    }

    public LegacyArangoDBContainer(Network network, ShardingClusterRole clusterRole) {
        super(
                        ...
                        .withDockerfileFromBuilder(
                                builder ->
                                        builder.from(DOCKER_IMAGE_NAME)
                                                ...
                                                .env("ARANGO_INITDB_ROOT_USERNAME", ARANGO_SUPER_USER)
                                                .env(
                                                        "ARANGO_INITDB_ROOT_PASSWORD",
                                                        ARANGO_SUPER_PASSWORD)
                                                .env("ARANGO_INITDB_DATABASE", "admin")
                                                .build()));
    ...
    public String getConnectionString(String username, String password) {
        return String.format(
                "mongodb://%s:%s@%s:%d",
                username, password, getContainerIpAddress(), getMappedPort(ARANGODB_PORT));
    }

    public String getHostAndPort() {
        return String.format("%s:%s", getContainerIpAddress(), getMappedPort(ARANGODB_PORT));
    }
    ...
    public String executeCommandFileInDatabase(String fileNameIgnoreSuffix, String databaseName) {
        ...
        final URL ddlTestFile = LegacyArangoDBContainer.class.getClassLoader().getResource(ddlFile);
            ...
            switch (clusterRole) {
                case CONFIG:
                    return String.format(
                            ...
                            ARANGODB_PORT, clusterRole.replicaSetName);
                case SHARD:
                    return String.format(
                ...
                case ROUTER:
                    return String.format(
                            ...
                            CONFIG.replicaSetName, CONFIG.hostname, ARANGODB_PORT);
                case NONE:
                default:
                    return String.format(
                            "mongod --port %d --replSet %s --keyFile /data/keyfile/random.key",
                            ARANGODB_PORT, NONE.replicaSetName);
            }
        }
    }
}
